//jaime1710
//Variables globales que cuentan puntuación, número movimientos y clicks del botón iniciar
var puntuacion = movimientos = clicks = 0;
//Animación que cambia el color del título a blanco
function colorTitulowhite(elemento){
  $(elemento).delay(1000).animate({
    //color:'#0000FF ' //comprobacion de color alterno
   color: '#fff'
 }, 10, function(){ // el numero 10 significa 10 milesimas de segundo se ejecuta la funcion
    colorTituloyellow(elemento);
  })
}
//Animación que cambia el color del título a amarillo
function colorTituloyellow(elemento){
  $(elemento).delay(1000).animate({
    color: '#DCFF0E'

  }, 10, function(){
    colorTitulowhite(elemento);
  })
}
...
function firstClickIniciar(){
  clicks++;
  if(clicks==1){
    var tiempo = 60 * 2; // Minutos para el juego seria  120 segundos
    $('.btn-reinicio').text('Reiniciar'); //codigo jQuery que dice en el selector coloque  el elemento Reiniciar
    llenarTodos();
    startTimer(tiempo, $("#timer"));
    postJugada();
  }else{
    location.reload(); //dice recargue el elemento actual
  }
